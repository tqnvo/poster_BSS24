...
\title{\scshape Mathematical\,modelling\,for\,all-solid-state\,battery:\,space-charge\,layer}
\author{
...
\begin{columns}
	...
	\block{State-of-the-art: Nucleation interface taking place at the unstable (SE|SSE)-Discontinuity}
	{
		...
		\end{minipage}
		%
		\begin{minipage}{0.107\textwidth}
			\begin{mdframed}
				\textbf{Solution:}
				Analysing system of PDEs arising in $(1)$ and $(2)$
				% eventually 
				results in
				a simplified Griffith nucleation criterion,
				including
				the Cauchy stress tensor denoted by $\sigma$,
				Young's elastic modulus $E_{Y}$, and
				experimentally-derived surface energy $\gamma_{s}$.
				Here, the $\sigma$ is computed 
				using FVM 
				based on OpenFOAM library 
				\cite{vo2023_obms23}\cite{vo2023_cse23}\cite{vo2022}\cite{vo2020}.
			\end{mdframed}
			%
			\vspace{-2mm}
			\begin{mdframed}
				Griffith criterion:
				\begin{align*}
					\sigma_{\text{Griffith}}^{\text{simplified}} \!=\! 
					\sqrt{\frac{2\,E_{Y}\,\gamma_{s}}{\pi\,a_{\text{crevice}}}}
				\end{align*}
			\end{mdframed}
		\end{minipage}
		%
