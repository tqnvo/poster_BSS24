...
\begin{columns}
	...
	\block{State-of-the-art: Nucleation interface taking place at the unstable (SE|SSE)-Discontinuity}
	{
		...
		\begin{minipage}{0.1\textwidth}
			...
			\begin{mdframed}
				\scalebox{0.7}{
					% Griffith nucleation criterion is computed based on
					% the relation
					% \begin{align*}
					% 	a_{\text{Griffith}}^{\text{simplified}}
					% 	= \sqrt{\frac{2\,E_{Y}\,\gamma_{s}}{\pi\,\sigma}},
					% \end{align*}
					% where the Cauchy stress $\sigma$
					% is computed based on OpenFOAM library.
					% Here, 
					% $E_{Y}$ is Young's modulus
					% and $\gamma_{s}$ surface energy.
					By analysing the system of equations in $(1)$ and $(2)$
					a simplified Griffith nucleation criterion is obtained. 
					This results in an expression
					% \begin{align*}
					% 	a_{\text{Griffith}}^{\text{simplified}} = 
					% 	\sqrt{\frac{2\,E_{Y}\,\gamma_{s}}{\pi\,\sigma}}
					% \end{align*}
					with Cauchy stress $\sigma$
					determined by the 
					OpenFOAM library. 
					In this formula, 
					$E_{Y}$ represents Young's modulus, and 
					$\gamma_{s}$ denotes surface energy.
				}
			\end{mdframed}
		\end{minipage}
		%
		\begin{mdframed}
			\textbf{Shock}: Li-ion transport leads to shocking phenomena at the discontinuous (SE$|$SSE)-Interface, influenced by the distribution and size of grains.
			\begin{center}				
