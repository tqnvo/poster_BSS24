	...
	\block{State-of-the-art: Nucleation interface taking place at the unstable (SE|SSE)-Discontinuity}
	{
		...
		\begin{minipage}{0.1\textwidth}
			\begin{mdframed}
				...
				% surface energy $\gamma_{s}$.
				% While $\gamma_{s}$ is taken from experiments,
				% $\sigma$ is computed based on the OpenFOAM library.
				\textbf{Solution:}
				Analysing equations $(1)$ and $(2)$ yields
				...
				with $\sigma$ calculated using the OpenFOAM library.
			\end{mdframed}
			%
			\begin{mdframed}
				\textbf{Griffith criterion}:
				\begin{align*}
