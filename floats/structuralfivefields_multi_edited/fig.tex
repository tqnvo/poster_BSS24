...
% \psfrag{c2}[c][c] {$\Bd_{E}$}
\psfrag{c2}[c][c] {\tiny $V_{e}$}
\psfrag{as}[c][c] {\tiny $a^{\!\ast}$}
\psfrag{xb}[c][c] {\tiny $\Bx \in \Omega$}
...
% Cation field
\psfrag{dispca}[c][c] {\scriptsize $\textsc{Cation } \text{Li}^{+} \textsc{ field }  c^{p}$}
\psfrag{cp}[c][c]  {$c^{p}$}
...
% Displacement field
\psfrag{disp}[c][c] {\tiny $\textsc{displacement field } \Bu$}
\psfrag{ud}[c][c]   {\tiny $\Bu$}
...
% Temperature field
\psfrag{temp}[c][c]  {\tiny $\textsc{temperature field } \theta$}
\psfrag{te}[c][c]    {\tiny $\theta$}
...
\psfrag{itf}[c][c]  {\scalebox{0.3}{$\textsc{(SE$|$SSE)-Interface}$}}
...
% Misorientation
\psfrag{misso}[c][c] {\tiny $\textsc{Electrostatic}\, V_{e}$}
\psfrag{po}[c][c]    {\tiny $\textsc{potential}\, V_{e}$}
...
\includegraphics[width=1.0\textwidth]{structuralfivefields_multi_edited.eps}